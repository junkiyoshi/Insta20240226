...
void ofApp::update() {
	...
	for (int i = 0; i < 32; i++) {
		...
		auto noise_location = glm::vec2(
			ofMap(ofNoise(39 + i * 1000, ofGetFrameNum() * 0.0085), 0, 1, -320, 320),
			ofMap(ofNoise(239 + i * 1000, ofGetFrameNum() * 0.0085), 0, 1, -320, 320));
		...
		auto next_noise_location = glm::vec2(
			ofMap(ofNoise(39 + i * 1000, (ofGetFrameNum() + 1) * 0.0085), 0, 1, -320, 320),
			ofMap(ofNoise(239 + i * 1000, (ofGetFrameNum() + 1) * 0.0085), 0, 1, -320, 320));
		...
		for (int k = 0; k < 2; k++) {
			...
			color.setHsb(i * 8, ofRandom(255), ofRandom(255));
...
void ofApp::draw() {
	...
	for (int i = 0; i < this->log_list.size(); i++) {
		...
		auto size = this->log_list[i].size() < 3 ? 5 : ofMap(this->log_list[i].size(), 3, 10, 5, 0);
